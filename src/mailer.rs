use budget::Budget;
use config::Email;
use handlebars::{Context, Handlebars, Helper, HelperDef, JsonRender, RenderContext, RenderError};
...
pub fn email_budget(cfg: &Email, budget: &Budget) {
    ...
    let builder = smtp::SmtpTransportBuilder::new((cfg.account.server.as_ref(), cfg.account.port))
                      .unwrap();
    let mut mailer = builder.credentials(cfg.account.username.as_ref(),
                                         cfg.account.password.as_ref())
...
impl HelperDef for MoneyHelper {
    fn call(&self,
            ...
            -> Result<(), RenderError> {
        let num = h.params().get(0).expect("BOD");
